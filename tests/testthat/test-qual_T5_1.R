...
testthat::test_that("Qual: Given appropriate raw participant-level data, flag values can be correctly assigned to records that meet flagging criteria, including custom thresholding (#159)", {
  ...
  expect_true(all(map_lgl(
    test_custom[outputs[!(outputs %in% c("vThreshold", "vFlag", "lAnalysis"))]],
    is.data.frame
  )))
  expect_equal(
    nrow(test_custom$Analysis_Flagged),
    nrow(test_custom$Analysis_Summary)
  )
  expect_identical(
    sort(test_custom$Analysis_Flagged$GroupID),
    sort(test_custom$Analysis_Summary$GroupID)
  )
  # expect_identical(test_custom$Analysis_Flagged$Flag, test_custom$Analysis_Summary$Flag)
  expect_identical(
    abs(hardcode_flag_custom$Flag),
    hardcode_flag_custom$hardcode_flag
  )
  ...
  expect_true(all(map_lgl(
    test_default[outputs[
      !(outputs %in% c("vThreshold", "vFlag", "lAnalysis"))
    ]],
    is.data.frame
  )))
  expect_equal(
    nrow(test_default$Analysis_Flagged),
    nrow(test_default$Analysis_Summary)
  )
  expect_identical(
    sort(test_default$Analysis_Flagged$GroupID),
    sort(test_default$Analysis_Summary$GroupID)
  )
  expect_identical(
    test_default$Analysis_Flagged$Flag,
    test_default$Analysis_Summary$Flag
  )
  expect_identical(
    abs(hardcode_flag_default$Flag),
    hardcode_flag_default$hardcode_flag
  )
})
