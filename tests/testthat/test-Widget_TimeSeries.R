test_that("Widget_TimeSeries creates a valid HTML widget {#166}", {
...
test_that("Widget_TimeSeries works with custom vOutcomeOptions {#166}", {
  reportingResults_filter <- gsm.core::reportingResults %>%
    dplyr::filter(MetricID == "Analysis_kri0001") %>%
...
test_that("Widget_TimeSeries assertions work {#166}", {
  reportingResults_filter <- gsm.core::reportingResults %>%
    dplyr::filter(MetricID == "Analysis_kri0001")
