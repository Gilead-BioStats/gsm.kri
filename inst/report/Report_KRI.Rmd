...
dfResults_delta <- dfResults_current %>%         
  dplyr::left_join(params$dfMetrics %>% select(MetricID, MetricName=Metric), by = "MetricID") %>%
  dplyr::mutate(MetricLabel = coalesce(MetricName, MetricID)) %>%
  add_Groups_metadata(
    params$dfGroups,
    setup$GroupLevel,
    setup$GroupLabelKey
  ) %>%
  mutate(GroupLabel = paste0(GroupID," (",.data[[setup$GroupLabelKey]],")"))
