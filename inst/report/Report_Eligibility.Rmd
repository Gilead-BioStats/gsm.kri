...
### Site
```{r, echo = FALSE, results = 'asis', fig.height=4}
eligibility_groupBar(dfNum = lListings[[1]] %>% dplyr::filter(Source != "Neither"), dfDenom = lListings[[1]], varGroupID = invid, strGroupLabel = "Site")
```

### Site (by %)
```{r, echo = FALSE, results = 'asis', fig.height=4}
eligibility_groupBar(
  dfNum = lListings[[1]] %>% dplyr::filter(Source != "Neither"),
  dfDenom = lListings[[1]],
  varGroupID = invid,
  strGroupLabel = "Site",
  bPercentage = TRUE
...
### Country
```{r, echo = FALSE, results = 'asis', fig.height=4}
eligibility_groupBar(dfNum = lListings[[1]] %>% dplyr::filter(Source != "Neither"), dfDenom = lListings[[1]], varGroupID = country, strGroupLabel = "Country")
```
